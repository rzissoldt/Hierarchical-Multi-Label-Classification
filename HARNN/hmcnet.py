...
class HmcNet(object):
    ...
    def __init__(
            ...
            fc_hidden_size, num_classes_list, total_classes, explicit_class_relation, l2_reg_lambda=0.0):
        ...
        self.beta = tf1.placeholder(tf1.float32, name="beta")
        ...
        #self.implicit_class_relations = tf1.Variable(tf1.truncated_normal(shape=[total_classes, total_classes],
                                                 #      stddev=0.1, dtype=tf1.float32), name="implicit_class_relations")
        def _attention(input_x, num_classes,visuals, name=""):
            """
            Attention Layer. Also known as TCCA Module.

            Args:
                input_x: [batch_size, feature_length]
                num_classes: The number of i th level classes.
                visuals: the hierarchichal information from the previous layer.
                name: Scope name.
            Returns:
            ...
            with tf1.name_scope(name + "attention"):
                ...
                attention_matrix = tf1.map_fn(
                    fn=lambda x: tf1.matmul(tf1.multiply(W_s2,visuals), x),
                    elems=tf1.tanh(
                        tf1.map_fn(
                ...
                attention_out_avg = tf1.reduce_mean(attention_out, axis=1)
                attention_weight_avg = tf1.nn.softmax(tf1.reduce_mean(attention_weight, axis=1))
            return attention_weight, attention_out_avg
        ...
        def _local_layer(input_x,current_num_classes,next_num_classes, name=""):
            """
            Local Layer. Used for Class Depency Module

            Args:
                input_x: [batch_size, fc_hidden_size]
                implicit_class_relations: [total_classes, total_classes]
                name: Scope name.
            Returns:
            ...
            with tf1.name_scope(name + "output"):
                ...
                implicit_class_relations = tf1.Variable(tf1.truncated_normal(shape=[current_num_classes, next_num_classes],
                                                    stddev=0.1, dtype=tf1.float32), name="W")
                visual = tf1.multiply(implicit_class_relations, tf1.expand_dims(scores, -1))
