...
class HybridPredictingModuleHighway(nn.Module):
    def __init__(self,num_layers,highway_num_layers,highway_fc_hidden_size,fc_hidden_size,total_classes,dropout_keep_prob,alpha):
        super(HybridPredictingModuleHighway,self).__init__()
        self.highway = HighwayLayer(input_size=highway_fc_hidden_size,num_layers=highway_num_layers)
        self.W_highway = nn.Parameter(truncated_normal(size=(highway_fc_hidden_size, fc_hidden_size*num_layers),std=he_weight_init(fc_hidden_size*num_layers)))
        self.b_highway = nn.Parameter(torch.ones(highway_fc_hidden_size)*he_weight_init(fc_hidden_size*num_layers))
        self.W_global_pred = nn.Parameter(truncated_normal(size=(total_classes,highway_fc_hidden_size),std=he_weight_init(highway_fc_hidden_size)))
        self.b_global_pred = nn.Parameter(torch.ones(total_classes)*he_weight_init(highway_fc_hidden_size))
        self.highway_drop = nn.Dropout(dropout_keep_prob)
        self.ham_out_batchnorm = nn.BatchNorm1d(highway_fc_hidden_size)
        self.ham_out_dropout = nn.Dropout1d(p=dropout_keep_prob)
        self.highway_batchnorm = nn.BatchNorm1d(highway_fc_hidden_size)
        self.alpha = alpha
...
class HmcNetLoss(nn.Module):
    def __init__(self,beta,explicit_hierarchy,device=None):
    ...
    def forward(self,x):
        ...
        loss = torch.sum(torch.stack([global_loss,local_loss,hierarchy_loss]))
        return loss, global_loss,local_loss,hierarchy_loss
