...
def calc_metrics(scores_list,labels_list,topK,pcp_hierarchy,pcp_threshold,threshold,num_classes_list,eval_pcp=False,eval_hierarchical_metrics=False,device=None):
    ...
    eval_macro_pre_ts = micro_precision(predicted_onehot_labels.to(dtype=torch.long),true_onehot_labels.to(dtype=torch.long))
    eval_macro_rec_ts = micro_recall(predicted_onehot_labels.to(dtype=torch.long),true_onehot_labels.to(dtype=torch.long))
    eval_macro_F1_ts = micro_f1_score(predicted_onehot_labels.to(dtype=torch.long),true_onehot_labels.to(dtype=torch.long))  
