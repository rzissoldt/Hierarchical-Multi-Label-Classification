...
def calc_metrics(scores_list,labels_list,topK,pcp_hierarchy,pcp_threshold,threshold,num_classes_list,eval_pcp=False,eval_hierarchical_metrics=False,device=None):
    ...
    true_onehot_labels = true_onehot_labels.to(dtype=torch.long)
    true_onehot_labels = true_onehot_labels.to(device=device)
    eval_micro_pre_ts = micro_precision(scores,true_onehot_labels)
    eval_micro_rec_ts = micro_recall(scores,true_onehot_labels)
    eval_micro_F1_ts = micro_f1_score(scores,true_onehot_labels)
    eval_macro_pre_ts = micro_precision(scores,true_onehot_labels)
    eval_macro_rec_ts = micro_recall(scores,true_onehot_labels)
    eval_macro_F1_ts = micro_f1_score(scores,true_onehot_labels)  
    eval_macro_auc = macro_auroc(scores,true_onehot_labels)
    eval_macro_auprc = macro_auprc(scores,true_onehot_labels)
    eval_micro_auc = micro_auroc(scores,true_onehot_labels)
    eval_micro_auprc = micro_auprc(scores,true_onehot_labels)
